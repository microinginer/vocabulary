...
class TranslateSentences extends Command
{
    protected $signature = 'translate:sentences {language} {--limit=100}';
    protected $description = 'Translate sentences from English to specified language using Yandex.Translate API';
    ...
    public function handle()
    {
        ...
        while (true) {
            $sentences = WordSentences::offset($offset)->limit($limit)->get();

            if ($sentences->isEmpty()) {
                break;
            }

            $texts = $sentences->pluck('content')->toArray();
                    ...
                    foreach ($sentences as $index => $sentence) {
                        ...
                        if ($translation) {
                            WordSentenceTranslation::updateOrCreate(
                                ['word_sentence_id' => $sentence->id, 'language' => $target_language],
